...
To quantify this directional influence, we introduce the Far From $\Vec{D}$ (FFD) variable, where $\Vec{D}$ represents the direction of minimum snow accumulation. Unlike the standard aspect, which references north (0°), FFD measures a slope's angular deviation from $\Vec{D}$ \textcolor{blue}{to better align} with the physical processes shaping snow distribution. %This approach aligns the coordinate system with the physical processes driving snow distribution establishing a meaningful reference direction.
...
Figure \ref{fig:LidarCover} \textcolor{blue}{shows} the three regions where the LiDAR surveys were conducted, \textcolor{blue}{highlighting} the maximum spatial extent of the resulting snow depth maps. \textcolor{blue}{The data used} are derived from two LiDAR data sources, described \textcolor{blue}{below}.
...
Additionally, the specific locations of our stations \textcolor{blue}{--situated} at elevations between 2000 and 3000 meters on flat, wind-sheltered terrain in avalanche-prone zones-- make it challenging to collect data that is broadly representative of the entire region. Relying \textcolor{blue}{solely} on data from the nearest station \textcolor{blue}{through} linear regression can therefore miss critical snow depth variations across such an extensive and topographically complex area. This limitation of DEMStat motivated our development of more sophisticated methods to capture temporal variability, namely the Voronoi and ProbStat approaches. These methods address spatial representation challenges and aim to improve snow depth mapping accuracy using available meteorological data.
...
As detailed in Section \ref{subsec:temporalvars}, we introduce temporal variability into our model by proposing three new features. \textcolor{blue}{While} linear regression (DEMStat) \textcolor{blue}{was} used in previous sections, here we test the last two features: Voronoi and ProbStat. We \textcolor{blue}{then} train two models with the Efficient Set. Incorporating Voronoi \textcolor{blue}{yields a validation} MAE of 0.527 m, \textcolor{blue}{while} ProbStat slightly improves \textcolor{blue}{the error} to 0.525 m. This results in a 0.93\% and 1.3\% \textcolor{blue}{reduction in error}, respectively, compared to the model trained solely with the Efficient Set.
...
\begin{figure}
  ...
  \caption{\textcolor{blue}{Differences between observed ($y_i$) and predicted ($\hat{y}_i$) snow depths across the Davos area (2017). Red color indicates underpredictions, blue color shows overpredictions, and green color represents accurate predictions.}}
...
For temporal generalization, while dynamic temporal variables enhance adaptability across the snow season, \textcolor{blue}{the study's temporal scope is currently limited, as the results are analyzed for only a single day. This constraint limits insights into how the framework performs over time.} Two key challenges affect temporal performance: (1) model training has primarily used peak snow conditions, \textcolor{blue}{which limits} accuracy during accumulation and melt phases; and (2) reliance on weather station data can hinder deployment in regions with sparse data coverage. \textcolor{blue}{Additionally,} the training data distribution \textcolor{blue}{poses challenges} for extreme snow depths, \textcolor{blue}{with} only 14\% of the samples \textcolor{blue}{representing} snow depths below 50 cm and 9.7\% above 3 m. \textcolor{blue}{As a result}, uncertainty increases for shallow ($<$0.5 m) and deep snow conditions, \textcolor{blue}{leading to a} tendency for underprediction at greater snow depths.
...
Several avenues for further study are identified. First, exploring and comparing alternative models and architectural variations within the U-Net framework \textcolor{blue}{could lead to} more effective snow depth mapping \textcolor{blue}{models}. Second, increasing spatial resolution could improve the accuracy of hydrological models \textcolor{blue}{based} on snow depth predictions. Third, incorporating additional dynamic input features, such as meteorological variables, could \textcolor{blue}{enhance} our understanding of snow dynamics and improve the predictive power of our model. \textcolor{blue}{Fourth}, gathering more comprehensive data on shallow and deep snow depths, \textcolor{blue}{including snow accumulation and melt phases, is crucial} for refining our model and ensuring its robustness across \textcolor{blue}{diverse} snow conditions and terrains. \textcolor{blue}{Finally,} applying the proposed methodology to estimate snow depth in other regions by retraining the network with local data offers exciting opportunities, \textcolor{blue}{particularly in areas where high-quality} reference data are available.
...
\section*{\textcolor{blue}{Data and code availability}}

\textcolor{blue}{This study integrates publicly available data, derived products, and proprietary commercial code and data protected under a non-disclosure agreement (NDA). For information on data access and code availability, please contact the corresponding author.}
