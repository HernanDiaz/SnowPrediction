"""
Genera dataset_v4_ms: tiles a 1m de resolucion con 22 canales (multi-escala).
=============================================================================

Extiende dataset_v4_17ch añadiendo 5 canales de contexto a 5m de resolucion:
los mismos canales topograficos basicos (DEM, Slope, Northness, Eastness, TPI)
extraidos del DEM a 5m y remuestreados a 256x256 con bilinear. El modelo
recibe a la vez el detalle fino a 1m Y el contexto de paisaje a 5m.
...
Canales (22):
  ...
  [17] DEM_5m       (elevacion a escala de paisaje)
  [18] Slope_5m     (pendiente suavizada)
  [19] Northness_5m
  [20] Eastness_5m
  [21] TPI_5m       (posicion topografica a escala de cuenca)

Split temporal (identico a v4_17ch):
  Train : 2021, 2022, 2023  (stride=128, 50% overlap)
  Val   : 2024               (stride=256, sin overlap)
  Test  : 2025               (stride=256, sin overlap)

Uso:
    .venv\\Scripts\\python.exe data/generate_dataset_v4_ms.py
"""
...
OUTPUT_DIR   = _REPO / "dataset_v4_ms"
...
def load_sx_arrays():
    """Carga los 8 rasters Sx_100m a 1m."""
...
def load_context_5m():
    ...
    Devuelve: (arrays_dict, transform_5m)
      arrays_dict: {'dem': ..., 'slope': ..., 'northness': ..., 'eastness': ..., 'tpi': ...}
    """
...
def extract_context_patch(ctx_arrays, transform_5m, bounds_1m, out_h, out_w):
    ...
    bounds_1m: (left, bottom, right, top) en CRS
    Devuelve: array (5, out_h, out_w)
    """
    ...
    for ci, key in enumerate(keys):
        ...
        # Upsample a resolucion del tile 1m
...
def main():
    ...
    with rasterio.open(DEM_1M_PATH) as src_dem:
        ...
        for fp in tqdm(lidar_files, desc="Generando tiles v4-ms"):
            fname    = fp.name
            ...
            # -----------------------------------------------------------------
            # D. Canales Sx (crop del raster global)
            # -----------------------------------------------------------------
            row_off = int(win.row_off)
            col_off = int(win.col_off)
            ...
            # -----------------------------------------------------------------
            bounds_crop = array_bounds(h, w, transform_crop)  # (left, bottom, right, top)
    ...
    csv_path = OUTPUT_DIR / "dataset_v4_ms.csv"
    df.to_csv(csv_path, index=False)

    print(f"\n{'='*60}")
    print(f"GENERACION COMPLETADA: {len(df)} tiles  |  22 canales")
    print(f"  Canales 0-16 : detalle 1m (DEM, Slope, N, E, TPI, SCE, Sx×8, Pers×3)")
