"""
Busqueda de hiperparametros con Optuna - Random Forest / dataset meteo1 (26 canales).
=======================================================================================

Dataset  : dataset_v4_ms_sx200_meteo (1m, 26 canales: 22 originales + 4 meteo)
...
Canales (26):
  ...
  [22] t2m_7d       [23] t2m_15d      [24] t2m_30d
  [25] ppAcc_from_oct
...
Salidas:
  - Modelo   : results/optuna_rf_meteo/rf_meteo_best.joblib
  - Metricas : results/optuna_rf_meteo/rf_meteo_metrics.json
  - Ranking  : results/optuna_rf_meteo/ranking_rf_meteo.json
  - BD Optuna: results/optuna_rf_meteo/optuna_rf_meteo.db

Uso:
    .venv\\Scripts\\python.exe baselines/optuna_rf_meteo.py
    .venv\\Scripts\\python.exe baselines/optuna_rf_meteo.py --trials 30
    .venv\\Scripts\\python.exe baselines/optuna_rf_meteo.py --resume
"""
...
# Configuracion
# ---------------------------------------------------------------------------
DATA_ROOT   = str(_REPO / 'dataset_v4_ms_sx200_meteo')
CSV_FILE    = os.path.join(DATA_ROOT, 'dataset_v4_ms_sx200_meteo.csv')
IMGS_DIR    = os.path.join(DATA_ROOT, 'images')
MASKS_DIR   = os.path.join(DATA_ROOT, 'masks')
RESULTS_DIR = str(_REPO / 'results/optuna_rf_meteo')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_rf_meteo.db'
STUDY_NAME  = 'rf_meteo_hpo_v1'
...
CHANNEL_NAMES = [
    ...
    't2m_7d', 't2m_15d', 't2m_30d', 'ppAcc_from_oct',
]
...
def normalize(X: np.ndarray) -> np.ndarray:
    """X: (N, 26) — replica SnowDataset._normalize()"""
...
print("\nCargando dataset_v4_ms_sx200_meteo...")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Optuna RF meteo | {args.trials} trials | 26 canales")
    print(f"{'='*60}\n")
    ...
    for k, v in sorted(fi.items(), key=lambda x: -x[1])[:8]:
        print(f"    {k:<20s}: {v:.4f}")

    model_path = os.path.join(RESULTS_DIR, 'rf_meteo_best.joblib')
    joblib.dump(rf_final, model_path)
    print(f"\n  Modelo guardado: {model_path}")

    result = {
        'experiment':   'rf_meteo_optuna',
        'dataset':      'dataset_v4_ms_sx200_meteo (1m, 26ch)',
    ...
    metrics_path = os.path.join(RESULTS_DIR, 'rf_meteo_metrics.json')
    ...
    ranking_path = os.path.join(RESULTS_DIR, 'ranking_rf_meteo.json')
    with open(ranking_path, 'w') as f:
        json.dump(ranking, f, indent=2)
    print(f"  Ranking guardado:   {ranking_path}")

    print(f"\n{'='*60}")
    print(f"  RF meteo TEST R2   = {test_metrics['R2']:.4f}")
    print(f"  RF meteo TEST RMSE = {test_metrics['RMSE']:.4f}")
    print(f"{'='*60}\n")
