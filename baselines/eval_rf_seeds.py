"""
Evalua RF con los mejores hiperparametros de optuna_rf_v4_ms_sx200 para seeds adicionales.
==========================================================================================
Mejores params (Trial 18):
  n_estimators=500, max_depth=10, min_samples_leaf=10,
  max_features='sqrt', min_samples_split=5

Uso:
    .venv/Scripts/python.exe baselines/eval_rf_seeds.py --seed 123
    .venv/Scripts/python.exe baselines/eval_rf_seeds.py --seed 7
"""
...
# Mejores hiperparametros de Trial 18
BEST_PARAMS = {
    'n_estimators':     500,
    'max_depth':        10,
    'min_samples_leaf': 10,
...
def main():
    ...
    parser.add_argument('--seed', type=int, required=True, help='Seed (e.g. 123 o 7)')
    args = parser.parse_args()

    SEED = args.seed
    RESULTS_DIR = str(_REPO / f'results/rf_v4_ms_sx200_s{SEED}')
    os.makedirs(RESULTS_DIR, exist_ok=True)

    print(f"\nRF 22ch | seed={SEED} | mejores params Trial 18")
    ...
    out = {
        'experiment': f'rf_v4_ms_sx200_s{SEED}',
        'seed': SEED,
        'best_params': BEST_PARAMS,
        'test_metrics': {k: float(v) for k, v in metrics.items()
                         if not isinstance(v, dict)},
    }
    out_path = os.path.join(RESULTS_DIR, f'rf_v4_ms_sx200_s{SEED}_metrics.json')
