"""
Busqueda de hiperparametros con Optuna - ResUNet++ / Dataset v4_ms_sx200 (22 canales).
========================================================================================

Arquitectura fija: ResUNet++ (GroupNorm + SE + ASPP + AttentionGates)
Dataset: dataset_v4_ms_sx200 (1m, 22ch, Sx_200m)
Split temporal: train=2021-2023 | val=2024 | test=2025

Espacio de busqueda (identico al de optuna_v6_resunetpp):
...
Fijado: loss=MSE, epochs=150, in_channels=22, num_groups=8, sin augmentation.
Pruning: MedianPruner (warm-up 10 trials, reporte cada 10 epocas desde ep20).
Storage: SQLite (resumible si se interrumpe).

Salidas:
  - Pesos por trial: Articulo 1/Models/optuna_resunetpp_v4ms/trial_NNN_b<base>_r2X.XXXX.pth
  - Ranking JSON:    results/optuna_resunetpp_v4ms/ranking_resunetpp_v4ms.json
  - BD Optuna:       results/optuna_resunetpp_v4ms/optuna_resunetpp_v4ms.db

Uso:
    .venv/Scripts/python.exe scripts/optuna/optuna_resunetpp_v4ms.py              # 60 trials
    .venv/Scripts/python.exe scripts/optuna/optuna_resunetpp_v4ms.py --trials 40
    .venv/Scripts/python.exe scripts/optuna/optuna_resunetpp_v4ms.py --resume
"""
...
# -- Configuracion fija --------------------------------------------------------
DATA_ROOT   = str(__import__('pathlib').Path(PROJECT_ROOT) / 'dataset_v4_ms_sx200')
CSV_FILE    = os.path.join(DATA_ROOT, 'dataset_v4_ms_sx200.csv')
IMGS_DIR    = os.path.join(DATA_ROOT, 'images')
MASKS_DIR   = os.path.join(DATA_ROOT, 'masks')
MODELS_DIR  = str(__import__('pathlib').Path(PROJECT_ROOT) / 'Articulo 1/Models/optuna_resunetpp_v4ms')
RESULTS_DIR = str(__import__('pathlib').Path(PROJECT_ROOT) / 'results/optuna_resunetpp_v4ms')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_resunetpp_v4ms.db'
STUDY_NAME  = 'resunetpp_v4ms_hpo_v1'
...
print("\nCargando splits de datos v4_ms_sx200 (ResUNet++)...")
...
def objective(trial):
    ...
    print(f"\n{'-'*65}", flush=True)
    print(f"[Trial {trial.number:03d}] ResUNet++ | base={base_ch} | "
...
def print_ranking(study, trial):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  RANKING ResUNet++ v4ms | {len(completed)} trials completados", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, 'ranking_resunetpp_v4ms.json'), 'w', encoding='utf-8') as f:
        json.dump(ranking_data, f, indent=2)


def main():
    parser = argparse.ArgumentParser(description='Optuna HPO ResUNet++ / dataset_v4_ms_sx200 (22ch)')
    ...
    # Warm-start: usar el mejor config de ResUNet++ v4_ms_sx200 como primer trial
    if n_existing == 0:
        best_resunetpp_params = {
            'base_channels': 48,
            'dropout_p':     0.1,
            'lr':            3.9e-04,
        ...
        study.enqueue_trial(best_resunetpp_params)
        print("Warm-start: trial 0 forzado con mejor config ResUNet++ v4_ms_sx200 (lr=3.9e-4, adam, gc=0.0)")

    print(f"\nEstudio: {STUDY_NAME}")
    print(f"Arquitectura: ResUNet++ (num_groups={NUM_GROUPS}) | Dataset: v4_ms_sx200 (22 canales, 1m)")
    ...
    print(f"\n{'='*65}")
    print(f"  ResUNet++ v4ms HPO COMPLETADO | {len(completed)} completados | {len(pruned)} pruned")
    if completed:
