"""
Optimizacion de hiperparametros con Optuna para ResUNet++ (dataset 5m).
=======================================================================
Espacio de busqueda:
...
Fijado: arch=resunetpp, num_groups=8, loss=MSE, epochs=150,
        sin SCE, sin augmentation, dataset 5m temporal split.
"""
...
MODELS_DIR  = str(__import__('pathlib').Path(PROJECT_ROOT) / 'Articulo 1/Models/optuna_resunetpp')
RESULTS_DIR = str(__import__('pathlib').Path(PROJECT_ROOT) / 'results/optuna_resunetpp')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_resunetpp_5m.db'
STUDY_NAME  = 'resunetpp_5m_hpo_v1'
...
def objective(trial):
    ...
    model = ResUNetPP(
        ...
        features=features, dropout_p=dropout_p, num_groups=NUM_GROUPS,
    ).to(DEVICE)
    ...
    print(f"\n{'-'*65}", flush=True)
    print(f"[Trial {trial.number:03d}] resunetpp | base={base_ch} | "
...
def print_ranking(study, trial):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  RANKING ResUNet++ tras {len(completed)} trials completados", flush=True)
    print(f"{'='*65}", flush=True)
    print(f"  {'#':<4} {'Trial':<6} {'val_loss':<10} {'test_R2':<10} "
          f"{'base':<6} {'lr':<8} {'wd':<8} {'bs':<4} {'opt':<6}", flush=True)
    print(f"  {'-'*4} {'-'*6} {'-'*10} {'-'*10} {'-'*6} {'-'*8} {'-'*8} {'-'*4} {'-'*6}", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, 'ranking_resunetpp.json'), 'w') as f:
...
def main():
    ...
    print(f"Arquitectura: ResUNet++ (GroupNorm + SE + ASPP + AttentionGates)")
    ...
    print(f"\n{'='*65}")
    print(f"  ResUNet++ HPO COMPLETADO | Trials: {len(completed)} completados, {len(pruned)} pruned")
    if completed:
