"""
Busqueda de hiperparametros con Optuna - UNet / Dataset meteo1 (26 canales).
=============================================================================

Arquitectura fija: UNet (BatchNorm)
...
Fijado: loss=MSE, epochs=50, in_channels=26, sin augmentation.
Pruning: MedianPruner (warm-up 10 trials, reporte cada 5 epocas desde ep10).
Storage: SQLite (resumible si se interrumpe).

Salidas:
  - Pesos por trial: Articulo 1/Models/optuna_unet_meteo/trial_NNN_b<base>_r2X.XXXX.pth
  - Ranking JSON:    results/optuna_unet_meteo/ranking_unet_meteo.json
  - BD Optuna:       results/optuna_unet_meteo/optuna_unet_meteo.db

Uso:
    .venv\\Scripts\\python.exe scripts/optuna/optuna_unet_meteo.py --trials 30
    .venv\\Scripts\\python.exe scripts/optuna/optuna_unet_meteo.py --resume
"""
...
MODELS_DIR  = str(__import__('pathlib').Path(PROJECT_ROOT) / 'Articulo 1/Models/optuna_unet_meteo')
RESULTS_DIR = str(__import__('pathlib').Path(PROJECT_ROOT) / 'results/optuna_unet_meteo')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_unet_meteo.db'
STUDY_NAME  = 'unet_meteo_hpo_v1'
...
print("\nCargando splits de datos meteo1 (UNet 26ch)...")
...
def objective(trial):
    ...
    print(f"\n{'-'*65}", flush=True)
    print(f"[Trial {trial.number:03d}] UNet 26ch | base={base_ch} | "
...
def print_ranking(study, trial):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  RANKING UNet meteo | {len(completed)} trials completados", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, 'ranking_unet_meteo.json'), 'w', encoding='utf-8') as f:
        json.dump(ranking_data, f, indent=2)


def main():
    parser = argparse.ArgumentParser(description='Optuna HPO UNet / dataset meteo1 (26ch)')
    ...
    # Warm-start: usar el mejor config del Optuna UNet anterior (22ch, Trial 11)
    if n_existing == 0:
        best_prev_params = {
            'base_channels': 48,
            'dropout_p':     0.0012,
            'lr':            3.064e-05,
            'weight_decay':  2.149e-05,
            'batch_size':    16,
            'optimizer':     'adam',
            'grad_clip':     1.0,
        }
        study.enqueue_trial(best_prev_params)
        print("Warm-start: trial 0 forzado con mejor config Optuna UNet 22ch (Trial 11)")

    print(f"\nEstudio: {STUDY_NAME}")
    print(f"Arquitectura: UNet | Dataset: meteo1 (26 canales)")
    ...
    print(f"\n{'='*65}")
    print(f"  UNet meteo HPO COMPLETADO | {len(completed)} completados | {len(pruned)} pruned")
    if completed:
