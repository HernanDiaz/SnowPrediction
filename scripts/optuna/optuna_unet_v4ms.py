"""
Busqueda de hiperparametros con Optuna - UNet / Dataset v4_ms_sx200 (22 canales).
==================================================================================
...
Fijado: loss=MSE, epochs=150, in_channels=22, sin augmentation.
Pruning: MedianPruner (warm-up 10 trials, reporte cada 10 epocas desde ep20).
Storage: SQLite (resumible si se interrumpe).

Salidas:
  - Pesos por trial: Articulo 1/Models/optuna_unet_v4ms/trial_NNN_b<base>_r2X.XXXX.pth
  - Ranking JSON:    results/optuna_unet_v4ms/ranking_unet_v4ms.json
  - BD Optuna:       results/optuna_unet_v4ms/optuna_unet_v4ms.db

Uso:
    .venv/Scripts/python.exe scripts/optuna/optuna_unet_v4ms.py              # 60 trials
    .venv/Scripts/python.exe scripts/optuna/optuna_unet_v4ms.py --trials 40
    .venv/Scripts/python.exe scripts/optuna/optuna_unet_v4ms.py --resume
"""
...
MODELS_DIR  = str(__import__('pathlib').Path(PROJECT_ROOT) / 'Articulo 1/Models/optuna_unet_v4ms')
RESULTS_DIR = str(__import__('pathlib').Path(PROJECT_ROOT) / 'results/optuna_unet_v4ms')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_unet_v4ms.db'
STUDY_NAME  = 'unet_v4ms_hpo_v1'
...
print("\nCargando splits de datos v4_ms_sx200 (UNet)...")
...
def print_ranking(study, trial):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  RANKING UNet v4ms | {len(completed)} trials completados", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, 'ranking_unet_v4ms.json'), 'w', encoding='utf-8') as f:
        json.dump(ranking_data, f, indent=2)


def main():
    parser = argparse.ArgumentParser(description='Optuna HPO UNet / dataset_v4_ms_sx200 (22ch)')
    ...
    print(f"\nEstudio: {STUDY_NAME}")
    print(f"Arquitectura: UNet | Dataset: v4_ms_sx200 (22 canales, 1m)")
    ...
    print(f"\n{'='*65}")
    print(f"  UNet v4ms HPO COMPLETADO | {len(completed)} completados | {len(pruned)} pruned")
    if completed:
