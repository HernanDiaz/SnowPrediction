"""
Busqueda de hiperparametros con Optuna - AttentionUNet / Dataset v6 (17 canales).
==================================================================================

Arquitectura fija: AttentionUNet
...
Salidas:
  - Pesos por trial: Articulo 1/Models/optuna_v6_attention_unet/trial_NNN_b<base>_r2X.XXXX.pth
  - Ranking JSON:    results/optuna_v6_attention_unet/ranking_attention_unet_v6.json
  - BD Optuna:       results/optuna_v6_attention_unet/optuna_attention_unet_v6.db

Uso:
    python optuna_v6_attention_unet.py               # 60 trials
    python optuna_v6_attention_unet.py --trials 40   # N trials
    python optuna_v6_attention_unet.py --resume      # Reanudar
"""
...
MODELS_DIR  = str(__import__('pathlib').Path(PROJECT_ROOT) / 'Articulo 1/Models/optuna_v6_attention_unet')
RESULTS_DIR = str(__import__('pathlib').Path(PROJECT_ROOT) / 'results/optuna_v6_attention_unet')
DB_PATH     = f'sqlite:///{RESULTS_DIR}/optuna_attention_unet_v6.db'
STUDY_NAME  = 'attention_unet_v6_hpo_v1'
...
print("\nCargando splits de datos v6 (AttentionUNet)...")
...
def objective(trial):
    ...
    print(f"\n{'-'*65}", flush=True)
    print(f"[Trial {trial.number:03d}] AttentionUNet | base={base_ch} | "
...
def print_ranking(study, trial):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  RANKING AttentionUNet v6 | {len(completed)} trials completados", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, 'ranking_attention_unet_v6.json'), 'w', encoding='utf-8') as f:
        json.dump(ranking_data, f, indent=2)


def main():
    parser = argparse.ArgumentParser(description='Optuna HPO AttentionUNet v6 (17 canales)')
    ...
    print(f"\nEstudio: {STUDY_NAME}")
    print(f"Arquitectura: AttentionUNet | Dataset: v6 (17 canales)")
    ...
    print(f"\n{'='*65}")
    print(f"  AttentionUNet v6 HPO COMPLETADO | {len(completed)} completados | {len(pruned)} pruned")
    if completed:
